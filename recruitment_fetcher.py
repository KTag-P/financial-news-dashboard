...
RECRUITMENT_KEYWORDS = ['채용', '공채', '신입', '인턴', '모집', '선발', '수시채용', '채용형']
EXCLUDED_KEYWORDS = ['투자', '유치', '선정', '솔루션', '플랫폼', '스타트업', 'MOU', '협약', '이노베이션', '펀드', '지원']
...
def fetch_recruitment_from_news(company_key):
    """Google News RSS로 채용 관련 뉴스 수집 (투자/제휴 뉴스 제외)."""
    config = COMPANIES.get(company_key)
    if not config:
        return []

    items = []
    # 검색 쿼리를 더 구체적으로 변경 ("신입", "공채", "인턴" 위주)
    queries = [
        f'"{config["name"]}" (신입 OR 공채 OR 인턴 OR 채용연계형)',
        f'"{config["name"]}" 채용',
    ]

    for query in queries:
        try:
            encoded_query = quote(f'{query} when:30d') # 기간도 30일로 단축
            ...
            for entry in feed.entries:
                ...
                # 1. 제외 키워드 강력 차단 (투자, 플랫폼 등)
                if any(bad in title for bad in EXCLUDED_KEYWORDS):
                    continue

                # 2. 필수 포함 키워드 확인 (회사명 + 채용관련)
